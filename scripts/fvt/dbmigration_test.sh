...
export ZIGLEDGER_CA_CLIENT_HOME="/tmp/db_migration/admin"
export ZIGLEDGER_CA_SERVER_HOME="$TESTDIR"
export CA_CFG_PATH="$TESTDIR"

###### SQLITE #####

mkdir -p $ZIGLEDGER_CA_SERVER_HOME
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'CREATE TABLE IF NOT EXISTS users (id VARCHAR(64), token bytea, type VARCHAR(64), affiliation VARCHAR(64), attributes TEXT, state INTEGER,  max_enrollments INTEGER);'
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'CREATE TABLE IF NOT EXISTS affiliations (name VARCHAR(64) NOT NULL UNIQUE, prekey VARCHAR(64));'
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'CREATE TABLE IF NOT EXISTS certificates (id VARCHAR(64), serial_number blob NOT NULL, authority_key_identifier blob NOT NULL, ca_label blob, status blob NOT NULL, reason int, expiry timestamp, revoked_at timestamp, pem blob NOT NULL, PRIMARY KEY(serial_number, authority_key_identifier));'
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "INSERT INTO affiliations (name) VALUES ('org1');"
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "INSERT INTO affiliations (name) VALUES ('org1.dep1');"
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "INSERT INTO certificates (id, serial_number, authority_key_identifier) VALUES ('registrar', '1234', '12345');"

# Start up the server and the schema should get updated
$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -D -d sqlite3
...
$SCRIPTDIR/zigledger-ca_setup.sh -K # Kill the server

# Check that the new schema took affect
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'pragma table_info(users)' > $TESTDIR/output.txt
...
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'SELECT value FROM properties WHERE (property = "identity.level")' | grep '1'
if [ $? != 0 ]; then
    ErrorMsg "Incorrect level found for 'identity.level' in properties table"
fi

sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'pragma table_info(affiliations)' > $TESTDIR/output.txt
...
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'SELECT value FROM properties WHERE (property = "affiliation.level")' | grep '1'
if [ $? != 0 ]; then
    ErrorMsg "Incorrect level found for 'affiliation.level' in properties table"
fi


sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'pragma table_info(certificates)' > $TESTDIR/output.txt
grep 'id|VARCHAR(255)' $TESTDIR/output.txt
if [ $? != 0 ]; then
    ErrorMsg "Database column 'id' should have character limit of 255"
fi
...
sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'SELECT value FROM properties WHERE (property = "certificate.level")' | grep '1'
if [ $? != 0 ]; then
    ErrorMsg "Incorrect level found for 'certificate.level' in properties table"
fi

rm $ZIGLEDGER_CA_SERVER_HOME/$DBNAME

###### MYSQL ######

$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -D -d mysql # Start up the server and the new schema should get created
$SCRIPTDIR/zigledger-ca_setup.sh -K # Kill the server
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -D -d mysql # Start up the server again and it should try to update the schema again, should result in no errors
if test $? -ne 0; then
...
# Start up the server and the schema should get updated
$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -D -d mysql
...
###### POSTGRES ######
$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -D -d postgres # Start up the server and the new schema should get created
$SCRIPTDIR/zigledger-ca_setup.sh -K # Kill the server
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -D -d postgres # Start up the server again and it should try to update the schema again, should result in no errors
if test $? -ne 0; then
    ErrorMsg "Failed to start up server that is using the latest database schema"
fi
$SCRIPTDIR/zigledger-ca_setup.sh -K # Kill the server
...
# Start up the server and the schema should get updated
$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -D -d postgres
