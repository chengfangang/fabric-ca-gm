...
function tableCount() {
   ...
   tableType="$2"
   shift 2
   $SCRIPTDIR/zigledger-ca_setup.sh -L -d $driver -D | sed -n "/$tableType:/,/^.*: *$/p" | sed '1,3d;$d' |
      awk -v s="$*" '
...
function genAffYaml() {
   export ZIGLEDGER_CA_CLIENT_HOME=$TESTDIR/admin
   local Planet=(0 1 2)
   local Landmass=(0 1)
   local Country=(0 1)
...
function setupServerEnv() {
   $SCRIPTDIR/zigledger-ca_setup.sh -d $dbDriver -I -S -X -n1 -D -x $TESTDIR > $TESTDIR/server.log 2>&1
   enroll
   # Ensure affiliations cannot be deleted if --cfg.affiliations.allowremove not configured
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove org1 $URI -H $TESTDIR/admin/ 2>&1|
      grep 'Authorization failure' ||
         ErrorMsg "should not be able to delete 'org1', or wrong error msg"
   $SCRIPTDIR/zigledger-ca_setup.sh -K

   # Generate a large affinity tree for testing;
   # this is way faster than adding with the cmd-line client
   genAffYaml >> $CA_CFG_PATH/runZigledgerCaFvt.yaml
   $SCRIPTDIR/zigledger-ca_setup.sh -d $dbDriver -S -X -n1 -D -x $TESTDIR -- \
   ...
   let dbEntries--
   serverEntries="$(( $($ZIGLEDGER_CA_CLIENTEXEC affiliation list $URI -H $TESTDIR/admin/ | wc -l)  -1))"
...
function testAffiliationRefs() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove org1 $URI -H $TESTDIR/admin/ -d 2>&1 |
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove org1.department1 $URI -H $TESTDIR/admin/ -d 2>&1 ||
         ErrorMsg "should be able to delete org1.department1"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove org1 --force $URI -H $TESTDIR/admin/ -d 2>&1 ||
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove bank_b $URI -H $TESTDIR/admin/ -d 2>&1 &&
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove bank_b $URI -H $TESTDIR/admin/ --force -d 2>&1 &&
...
function testAllowremove() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove bank_b --force $URI -H $TESTDIR/admin/ 2>&1 |
      grep 'Authorization failure' ||
   ...
   # try again
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove bank_b --force $URI -H $TESTDIR/admin/ 2>&1  || ErrorMsg "should be able to delete 'bank_b'"
   # make sure entries are deleted
   $SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver -D | grep bank_b && ErrorMsg "'bank_b' not deleted"
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove planet2.landmass1 --force $URI -H $TESTDIR/admin/ 2>&1 ||
      ErrorMsg "should be able to delete 'planet2.landmass1'"
   # make sure entries are deleted
   $SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver -D | grep "planet2.landmass1" && ErrorMsg "'planet2.landmass1' not deleted"
...
function testAffiliationMgr() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove org2 $URI -H $TESTDIR/admin2 2>&1 |
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity add affman $URI -H $TESTDIR/admin --secret passwd \
          --attrs '"hf.Registrar.Roles=client,user,peer,validator,auditor,ca"' \
          --affiliation "planet2.landmass0.country1.province0"
   $ZIGLEDGER_CA_CLIENTEXEC identity modify affman --attrs "hf.AffiliationMgr=1" $URI -H $TESTDIR/admin
   enroll affman passwd
}

function testTreePruningFailCases() {
   # higher
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove planet2.landmass0.country1 $URI -H $TESTDIR/affman/ -d 2>&1 |
      grep 'Authorization failure' || ErrorMsg "Should not be able to delete 'planet2.landmass0.country1"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation add planet2.landmass0.country10 $URI -H $TESTDIR/affman/ -d 2>&1 |
      grep 'Authorization failure' || ErrorMsg "Should not be able to add 'planet2.landmass0.country10"
   # lateral
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove planet2.landmass0.country1.province1 $URI -H $TESTDIR/affman/ -d 2>&1 |
      grep 'Authorization failure' || ErrorMsg "Should not be able to delete 'planet2.landmass0.country1.province1"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation add planet2.landmass0.country1.province10 $URI -H $TESTDIR/affman/ -d 2>&1 |
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove planet2.landmass0.country1.province0 $URI -H $TESTDIR/affman/ -d 2>&1 |
   ...
   for l in 0 1; do for c in 0 1 2; do for h in 0 1 2 3; do
      $ZIGLEDGER_CA_CLIENTEXEC identity add newuser$l$c$h $URI -H $TESTDIR/admin --secret passwd \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation modify planet2 --name planetX $URI -H $TESTDIR/admin 2>&1 |
      grep "Need to use 'force'" || ErrorMsg "Should not be able to modify affiliation w/o --force"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation modify planet2 --name planetX --force $URI -H $TESTDIR/admin ||
         ErrorMsg "Should be able to modify affiliation w/ --force"
   displayRunningTotal
   verifyTotals "403 403 0 202" || ErrorMsg "Wrong number of affiliations"
   test $($SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver | grep planetX | wc -l) -eq 25 ||
      ErrorMsg "Wrong number of users"
}

function testTreePruningSuccessCases() {
   # lower, succeeds -- all children deleted
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove --force planetX.landmass0.country1.province0.locale0 $URI -H $TESTDIR/affman ||
      ErrorMsg "Should be able to delete lower affiliation"
   displayRunningTotal
   verifyTotals "403 403 0 186" || ErrorMsg "Wrong number of affiliations"
   test $($SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver | grep planetX | wc -l) -eq 13 ||
      ErrorMsg "Wrong number of users"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation add planetX.landmass0.country1.province0.locale10 $URI -H $TESTDIR/affman ||
   ...
   test $($SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver | grep planetX | wc -l) -eq 13 ||
      ErrorMsg "Wrong number of users"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation modify planetX.landmass0.country1.province0.locale10 \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity modify affman --attrs "hf.AffiliationMgr=T" $URI -H $TESTDIR/admin || ErrorMsg "Failed to update affman"
   $ZIGLEDGER_CA_CLIENTEXEC affiliation remove --force planetX.landmass0.country1.province0.locale1 $URI -H $TESTDIR/affman/ ||
      ErrorMsg "Should be able to delete lower affiliation"
   displayRunningTotal
   verifyTotals "403 403 0 171" || ErrorMsg "Wrong number of affiliations"
   test $($SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver | grep planetX | wc -l) -eq 1 ||
      ErrorMsg "Wrong number of users"
...
function testCertRevocation() {
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -L -d $dbDriver -D |
      awk -F'|' -v revoked=0 '
...
function testAlternateTruthValues() {
   for v in 0 F false; do
      $ZIGLEDGER_CA_CLIENTEXEC identity modify affman --attrs "hf.AffiliationMgr=$v" $URI -H $TESTDIR/admin
      $ZIGLEDGER_CA_CLIENTEXEC affiliation add planetX.landmass0.country1.province0.locale1.village $URI -H $TESTDIR/affman 2>&1 |
         grep "Caller has a value of 'false'" || ErrorMsg "Should have failed Authorization"
      $ZIGLEDGER_CA_CLIENTEXEC affiliation remove --force planetX.landmass0.country1.province0.locale1 $URI -H $TESTDIR/affman 2>&1 |
         grep "Caller has a value of 'false'" || ErrorMsg "Should have failed Authorization"
      $ZIGLEDGER_CA_CLIENTEXEC affiliation modify planetX.landmass0.country1.province0.locale1 \
...
function testNonExistant() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation add org1 $URI -H $TESTDIR/admin/ -d 2>&1 |
   ...
   $ZIGLEDGER_CA_CLIENTEXEC affiliation modify plan9 --name castleBravo $URI -H $TESTDIR/admin/ -d 2>&1 |
      grep '404 Not Found' ||
         ErrorMsg "should not be able to add 'plan9'"
   # Attempt to list an affiliation that doesn't exist
   $ZIGLEDGER_CA_CLIENTEXEC affiliation list --affiliation plan9 $URI -H $TESTDIR/admin/ -d 2>&1 |
...
$SCRIPTDIR/zigledger-ca_setup.sh -D -R -x $TESTDIR
mkdir -p $TESTDIR
setTLS
