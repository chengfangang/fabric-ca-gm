...
$SCRIPTDIR/zigledger-ca_setup.sh -R -x $CA_CFG_PATH -d mysql
$SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -d mysql
enroll
...
$SCRIPTDIR/zigledger-ca_setup.sh -L -d mysql
$SCRIPTDIR/zigledger-ca_setup.sh -R -x $CA_CFG_PATH -d mysql
rm -rf $ZIGLEDGER_CA_CLIENT_HOME
kill $HTTP_PID
wait $HTTP_PID
CleanUp $RC
exit $RC
