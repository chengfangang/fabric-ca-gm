...
DEFAULT_CA_CONFIG=zigledger-ca-config.yaml
RC=0
DBNAME=zigledger_ca
INTDBNAME=intzigledger_ca
...
setTLS
export SHELL PROXY_PORT ROOTDIR ITERATIONS USERNAME PROTO TLSOPT ZIGLEDGER_CA_CLIENTEXEC ENROLLCERT
...
function enrollAdmin() {
   ...
   ZIGLEDGER_CA_CLIENT_HOME=$dir/admin$ca \
   $ZIGLEDGER_CA_CLIENTEXEC enroll --debug $TLSOPT \
     -u ${PROTO}admin:adminpw@localhost:$port \
     --csr.hosts admin@fab-client.raleigh.ibm.com \
     --csr.hosts admin.zigledger.raleigh.ibm.com,127.0.0.2 \
...
function revokeUsers() {
   ...
   ZIGLEDGER_CA_CLIENT_HOME=$dir/admin$ca \
   $ZIGLEDGER_CA_CLIENTEXEC revoke --gencrl --debug $TLSOPT \
     -u ${PROTO}admin:adminpw@localhost:$port \
...
function reenrollUsers() {
   ...
   touch $dir/admin$ca/log.txt
   ZIGLEDGER_CA_CLIENT_HOME=$dir/${USERNAME}${ca}-${user} \
   $ZIGLEDGER_CA_CLIENTEXEC reenroll --debug $TLSOPT \
...
function register() {
   ...
   : ${port:=$PROXY_PORT}
   ZIGLEDGER_CA_CLIENT_HOME=$dir/admin$ca \
   $ZIGLEDGER_CA_CLIENTEXEC register --debug -u ${PROTO}localhost:$port $TLSOPT \
...
function DBvalidateUsers() {
   ...
   DBNAME=$dbname $SCRIPTDIR/zigledger-ca_setup.sh -L -d $DRIVER \
...
for DRIVER in mysql postgres; do
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -x $ROOTDIR -R -u $NUMCAS
   DBNAME=$INTDBNAME $SCRIPTDIR/zigledger-ca_setup.sh -x $ROOTDIR -R -u $NUMCAS
   rm -rf $ROOTDIR
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -x $ROOTDIR -I -n 1 -u $NUMCAS \
                                 -n $NUMSERVER -D -d $DRIVER > $ROOTDIR/log.txt 2>&1
   echo -e "   >>>>>>>>>>  Initializing Intermediate CAs"
   DBNAME=$INTDBNAME $SCRIPTDIR/zigledger-ca_setup.sh -D -d $DRIVER -u $NUMCAS -I -r $INTERMEDIATE_CA_DEFAULT_PORT -x $INTDIR \
...
EOF
   ...
   sed -i 's/cacount:.*/cacount:/g
           ...
           s/pathlength:.*/pathlength:/g' $ROOTDIR/runZigledgerCaFvt.yaml $INTDIR/runZigledgerCaFvt.yaml
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -N -X -x $ROOTDIR -S -n $NUMSERVER -D -d $DRIVER \
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -n $NUMSERVER -S -r $INTERMEDIATE_CA_DEFAULT_PORT -x $INTDIR \
   ...
   for SERVER in $PROXY_PORT $INTERMEDIATE_PROXY_PORT ; do
      ...
      for ca in $(seq $NUMCAS); do
         ...
         for user in $(seq $((ITERATIONS/2)) ); do
            echo "ZIGLEDGER_CA_CLIENT_HOME=$userdir/admin$ca $ZIGLEDGER_CA_CLIENTEXEC getcacert --debug -u ${PROTO}localhost:$SERVER $TLSOPT --caname ca$ca > $userdir/admin$ca/cacert.txt 2>&1" >> /tmp/cmd.lst
         done
         ...
         for user in $(seq $((ITERATIONS/2+1)) $ITERATIONS); do
            echo "ZIGLEDGER_CA_CLIENT_HOME=$userdir/admin$ca $ZIGLEDGER_CA_CLIENTEXEC gencrl --debug -u ${PROTO}localhost:$SERVER $TLSOPT --caname ca$ca > $userdir/admin$ca/crl.txt 2>&1" >> /tmp/cmd.lst
         done
      done
      ...
      for ca in $(seq $NUMCAS); do
         ZIGLEDGER_CA_CLIENT_HOME=$userdir/admin$ca $ZIGLEDGER_CA_CLIENTEXEC gencrl --debug -u ${PROTO}localhost:$SERVER $TLSOPT --caname ca$ca > $userdir/admin$ca/crl.txt 2>&1
