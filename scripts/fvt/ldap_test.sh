...
$SCRIPTDIR/zigledger-ca_setup.sh -I -a -D -X -S -n1
...
revokeEcert() {
   ...
   case "$result" in
      pass) echo "User '$admin' is revoking the ecert of user cert of user '$user' ..."
            $ZIGLEDGER_CA_CLIENTEXEC revoke -u $URI -a $AKI -s $SN $TLSOPT -H $UDIR/$admin 2>&1
...
# Caller does not have authority to act on affiliation
            #$ZIGLEDGER_CA_CLIENTEXEC revoke -u $URI -a $AKI -s $SN $TLSOPT -H $UDIR/$admin 2>&1| grep 'does not have authority to revoke'
            $ZIGLEDGER_CA_CLIENTEXEC revoke -u $URI -a $AKI -s $SN $TLSOPT -H $UDIR/$admin 2>&1| egrep "(does not have authority to (act|revoke)|Authorization failure)"
...
# User 'tstadmin' (uid=tstadmin,ou=tst,ou=zigledger,dc=hyperledeger,dc=example,dc=com)
...
# User 'devadmin' (uid=devadmin,ou=dev,ou=zigledger,dc=hyperledeger,dc=example,dc=com)
...
$ZIGLEDGER_CA_CLIENTEXEC gencrl -u $URI -H $UDIR/admin $TLSOPT
...
$ZIGLEDGER_CA_CLIENTEXEC gencrl -u $URI -H $UDIR/notadmin $TLSOPT 2>&1| grep 'Authorization failure'
