...
function enrollUser() {
   ...
   export ZIGLEDGER_CA_HOME=$KEYSTORE/$REGISTRAR
   OUT=$($SCRIPTDIR/register.sh -u $USERNAME -t $USERTYPE -a "$ATTR" -x $ZIGLEDGER_CA_HOME)
   echo "$OUT"
   PASSWD="$(echo $OUT | tail -n1 | awk '{print $NF}')"
   echo "PASSWD: $PASSWD"
   export ZIGLEDGER_CA_HOME=$KEYSTORE/$USERNAME
   $SCRIPTDIR/enroll.sh -u $USERNAME -p $PASSWD -x $ZIGLEDGER_CA_HOME
}
...
function registerUser() {
   ...
   mkdir -p $KEYSTORE/$USERNAME
   export ZIGLEDGER_CA_HOME=$KEYSTORE/$REGISTRAR
   $SCRIPTDIR/register.sh -u $USERNAME -t $USERTYPE -a "$ATTR" -x $ZIGLEDGER_CA_HOME 2>&1
   test $? -ne 0 && return 1
}
...
export ZIGLEDGER_CA_DEBUG
mkdir -p $KEYSTORE/$REGISTRAR
export ZIGLEDGER_CA_HOME=$KEYSTORE/$REGISTRAR

#for driver in sqlite3 postgres mysql; do
for driver in sqlite3 ; do
   $SCRIPTDIR/zigledger-ca_setup.sh -R -x $ZIGLEDGER_CA_HOME
   $SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -d $driver
   if test $? -ne 0; then ErrorMsg "server setup failed"; continue; fi
   $SCRIPTDIR/enroll.sh -u $REGISTRAR -p $REGIRSTRARPWD -x $ZIGLEDGER_CA_HOME
