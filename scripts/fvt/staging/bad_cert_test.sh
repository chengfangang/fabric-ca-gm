...
RC=0
export ZIGLEDGER_CA_HOME="$HOME/zigledger-ca"
CERT_HOME="/tmp/CAs/"
CLIENT_CERT="$ZIGLEDGER_CA_HOME/cert.pem"
CLIENT_KEY="$ZIGLEDGER_CA_HOME/key.pem"
...
for CA in NOT_A_CA BAD_END_CA BAD_START_CA INVALID_USE UNSUPPORTED; do
   for driver in sqlite3 ; do
      ...
      $SCRIPTDIR/zigledger-ca_setup.sh -m 0 -I -S -X -c $SERVER_CERT -k $SERVER_KEY
      $($ZIGLEDGER_CA_DEBUG) && printAuth $SERVER_CERT $SERVER_KEY
      ...
      prevId="$currId"
      $($ZIGLEDGER_CA_DEBUG) && printAuth $CLIENT_CERT $CLIENT_KEY
