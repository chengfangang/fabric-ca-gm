...
function rmConfigFiles() {
   rm -rf $TESTDIR/ca-cert.pem \
          $TESTDIR/zigledger-ca-server-config.yaml \
          $TESTDIR/zigledger-ca-server.db $TESTDIR/msp \
          $TESTDIR/zigledger-ca-cert.pem $TESTDIR/zigledger_ca \
          $TESTDIR/runZigledgerCaFvt.yaml
}
...
function passWordSub() {
   sed -i "/datasource:/ s/\(password=\)[[:alnum:]]\+\(.*\)/\1$PSWD\2/
          ...
          s/datasource:\(.*\)mysql@/datasource:\1$PSWD@/" $TESTDIR/runZigledgerCaFvt.yaml
}

function testBootstrap() {
   > $LOGFILE
   # Test using bootstrap ID
   zigledger-ca-server init -b $USER:$PSWD -d 2>&1 | tee $LOGFILE
   test ${PIPESTATUS[0]} -eq 0 && checkPasswd "$PSWD" || ErrorMsg "Init of CA failed"
   cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/testBootstrap.log
}

function testCaRegistry() {
   > $LOGFILE
   # Test using multiple IDs from pre-supplied config file
   $SCRIPTDIR/zigledger-ca_setup.sh -I -X -n1 -D 2>&1 | tee $LOGFILE
   test ${PIPESTATUS[0]} -eq 0 && checkPasswd "$PSWD" || ErrorMsg "Init of CA failed"
   cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/testCaRegistry.log
}
...
function testExternalServers() {
   for server in mysql postgres ldap; do
      rmConfigFiles
      case $server in
         ldap) $SCRIPTDIR/zigledger-ca_setup.sh -a -I -D > $LOGFILE 2>&1 ;;
            *) $SCRIPTDIR/zigledger-ca_setup.sh -I -D -d $server > $LOGFILE 2>&1 ;;
      esac
      passWordSub
      $SCRIPTDIR/zigledger-ca_setup.sh -D -X -S >> $LOGFILE 2>&1
      test $? -eq 0 && checkPasswd "$PSWD" $server || ErrorMsg "Start of CA failed"
      cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/test${server}.log
...
function testRegister() {
   rmConfigFiles
   $SCRIPTDIR/zigledger-ca_setup.sh -D -X -I -S > $LOGFILE 2>&1
   test $? -eq 0 && checkPasswd "$PSWD" $server || ErrorMsg "Start of CA failed"
   enroll
   register
   checkPasswd "" register
   cat $LOGFILE
   cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/testRegisterGeneratedPswd.log
   > $LOGFILE
   register "" Testuser2 "" "" "" "" "$PSWD"
   checkPasswd "$PSWD" register
   cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/testRegisterSuppliedPswd.log
}

function testIntermediateCa() {
   ZIGLEDGER_CA_SERVER_HOME="$ZIGLEDGER_CA_SERVER_HOME/intCa1"
   LOGFILE=$TESTDIR/testIntermediateCa.log
   zigledger-ca-server start --csr.hosts 127.0.0.2 --address 127.0.0.2 --port 7055 -b admin:adminpw $INTTLSOPT \
                          -u ${PROTO}intermediateCa1:intermediateCa1pw@127.0.0.1:$CA_DEFAULT_PORT -d > $LOGFILE 2>&1 &
   pollZigledgerCa "" 127.0.0.2 7055 || ErrorMsg "Failed to start intermediate CA"
   checkPasswd intermediateCa1pw intermediateCa
   cp $LOGFILE $ZIGLEDGER_CA_SERVER_HOME/testIntermediateCa.log
}
...
LOGFILE=$ZIGLEDGER_CA_SERVER_HOME/log.txt
