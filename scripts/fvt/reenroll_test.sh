...
export CA_CFG_PATH
export ZIGLEDGER_CA_CLIENT_HOME="$CA_CFG_PATH/admin"
...
for driver in sqlite3 postgres mysql; do
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -R -d $driver -x $CA_CFG_PATH
   $SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -n $NUM_SERVERS -d $driver -x $CA_CFG_PATH
   if test $? -ne 0; then
