...
TESTCONFIG="$ZIGLEDGER_CA_SERVER_HOME/testconfig.yaml"
DBNAME=zigledger_ca
...
function resetDB {
  case "$driver" in
    sqlite3)
      rm -rf $ZIGLEDGER_CA_SERVER_HOME/$DBNAME ;;
    postgres)
...
function loadUsers {
  case "$driver" in
    sqlite3)
      mkdir -p $ZIGLEDGER_CA_SERVER_HOME
      sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME 'CREATE TABLE IF NOT EXISTS users (id VARCHAR(255), token bytea, type VARCHAR(256), affiliation VARCHAR(1024), attributes TEXT, state INTEGER,  max_enrollments INTEGER);'
      sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "INSERT INTO users (id, token, type, affiliation, attributes, state, max_enrollments)
...
function validateUsers {
  ...
  case "$driver" in
    sqlite3)
      sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "SELECT attributes FROM users WHERE (id = 'registrar');" | grep '"name":"hf.Registrar.Attributes","value":"*"'
      if test $? -eq 1; then
        ErrorMsg "Failed to correctly migrate user 'registar' on sqlite"
      fi
      sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "SELECT attributes FROM users WHERE (id = 'notregistrar');" | grep '"name":"hf.Registrar.Attributes","value":"*"'
      if test $? -eq 0; then
        ErrorMsg "Failed to correctly migrate user 'notregistar' on sqlite"
      fi
      sqlite3 $ZIGLEDGER_CA_SERVER_HOME/$DBNAME "SELECT attributes FROM users WHERE (id = 'a');" | grep '"name":"hf.Registrar.Attributes","value":"*"'
...
genConfig "9.9.9.9"
zigledger-ca-server start -b a:b -c $TESTCONFIG -d
if test $? -ne 1; then
...
for driver in sqlite3 postgres mysql; do
  ...
  case "$driver" in
  sqlite3)
    rm -rf $ZIGLEDGER_CA_SERVER_HOME
    mkdir -p $ZIGLEDGER_CA_SERVER_HOME
    sqlite3 $ZIGLEDGER_CA_SERVER_HOME/zigledger_ca 'CREATE TABLE IF NOT EXISTS properties (property VARCHAR(255), value VARCHAR(256), PRIMARY KEY(property));'
    sqlite3 $ZIGLEDGER_CA_SERVER_HOME/zigledger_ca 'INSERT INTO properties (property, value) Values ("identity", "9");'
    ;;
  postgres)
    psql -d postgres -c "DROP DATABASE zigledger_ca"
    psql -d postgres -c "CREATE DATABASE zigledger_ca"
    psql -d zigledger_ca -c "CREATE TABLE IF NOT EXISTS properties (property VARCHAR(255), value VARCHAR(256), PRIMARY KEY(property))"
    psql -d zigledger_ca -c "INSERT INTO properties (property, value) Values ('identity', '9')"
    ;;
  mysql)
    mysql --host=localhost --user=root --password=mysql -e "DROP DATABASE zigledger_ca"
    mysql --host=localhost --user=root --password=mysql -e "CREATE DATABASE zigledger_ca"
    mysql --host=localhost --user=root --password=mysql --database=zigledger_ca -e "CREATE TABLE IF NOT EXISTS properties (property VARCHAR(255), value VARCHAR(256), PRIMARY KEY(property))"
    mysql --host=localhost --user=root --password=mysql --database=zigledger_ca -e "INSERT INTO properties (property, value) Values ('identity', '9')"
    ;;
  *)
    echo "Invalid database type"
    exit 1
  ...
  $SCRIPTDIR/zigledger-ca_setup.sh -I -D -d $driver
  if ! test $? -eq 0; then
