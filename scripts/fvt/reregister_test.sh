...
export CA_CFG_PATH
export ZIGLEDGER_CA_CLIENT_HOME="$CA_CFG_PATH/$REGISTRAR"
...
for driver in sqlite3 postgres mysql; do
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -I -S -X -n $NUM_SERVERS -t rsa -l 2048 -d $driver -x $CA_CFG_PATH
   if test $? -ne 0; then
      ErrorMsg "Failed to setup zigledger-ca server"
      continue
   fi
