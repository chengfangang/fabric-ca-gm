...
function registerEnroll() {
   roles="role$1"
   utype="type$1"
   eval $ZIGLEDGER_CA_CLIENTEXEC identity add userType$i $URI --secret userType${i}pw \
...
function getAttrs() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity list $URI -H $TESTDIR/$admin 2>&1 |
     grepPrint "^Name: $user," |
     grep -oP "Attributes:.*?]"|
     tr '{' "\n" |
...
function testAuthenticationAuthorization() {
   ...
   /etc/zhigui/zigledger-ca/pki -f newcert -t ec -l 256 \
   ...
   for op in list remove add modify; do
      ...
      # Unknown CA
      $ZIGLEDGER_CA_CLIENTEXEC identity $op $user $URI -d -H $TESTDIR/admin 2>&1 |
      ...
      $ZIGLEDGER_CA_CLIENTEXEC identity $op $user $URI -d -H $TESTDIR/testUser 2>&1 |
         # @TODO these messages need to change
...
function testRoleAuthorization() {
   ...
   enroll
   # Baseline
   $ZIGLEDGER_CA_CLIENTEXEC identity list $URI -H $TESTDIR/admin ||
     ErrorMsg "admin 'identity list' failed"
   # the type of the identity being added must be in the user's hf.Registrar.Roles list
   $ZIGLEDGER_CA_CLIENTEXEC identity add userType1 $URI -H $TESTDIR/admin \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity modify admin $URI -H $TESTDIR/admin/ -d \
   ...
   for i in  $(seq $NUMROLES); do
      ...
      test $($ZIGLEDGER_CA_CLIENTEXEC identity list $URI -H $TESTDIR/userType$i 2>&1 | wc -l) -eq 1 ||
         ErrorMsg "userType$i 'identity list' failed"
      $ZIGLEDGER_CA_CLIENTEXEC identity list $URI -H $TESTDIR/userType$i 2>&1 |
...
function testModifyRegistrarRoles() {
   ...
   # should fail
   $ZIGLEDGER_CA_CLIENTEXEC identity modify userType1 $URI -d \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity modify admin2 $URI -d -H $TESTDIR/admin \
      --attrs '"hf.Registrar.Roles=client,user,peer,validator,auditor,ca,app,role1,role2,role3,role4,role5,role6,role7,role8,apple,orange"' ||
...
function testDelegation () {
   ...
   # $ZIGLEDGER_CA_CLIENTEXEC identity add userType10 $URI -d -H $TESTDIR/admin --type role1 --affiliation ${defaultValues[Affiliation]} --attrs '"hf.Registrar.DelegateRoles=type10"' | grepPrint "403 Forbidden" || ErrorMsg "admin should not be able to add user with type 'type10', or wrong error code"
   $ZIGLEDGER_CA_CLIENTEXEC identity add userType10 $URI -d -H $TESTDIR/admin \
...
restrictedAdminAttrsAttrs='
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity add restrictedAdmin $URI -d \
      --json "$restrictedAdminAttrsAttrs" -H $TESTDIR/admin 2>&1
   pw=superUserpw
   enroll restrictedAdmin $pw
   $ZIGLEDGER_CA_CLIENTEXEC identity list $URI -d --id restrictedAdmin -H $TESTDIR/restrictedAdmin
   if test "$?" -ne 0; then
...
SuperAttrs='
   ...
   [{"name": "hf.Registrar.Roles", "value": "pianist,SuperUser,client,user,peer,validator,auditor,ca,app,role1,role2,role3,role4,role5,role6,role7,role8,apple,orange"},
    ...
    {"name": "hf.GenCRL", "value": "true"},
    {"name": "hf.Registrar.Attributes", "value": "*"}]}'
   $ZIGLEDGER_CA_CLIENTEXEC identity add SuperUser $URI -d \
...
function testDefaults() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity add vanillaUser $URI -H $TESTDIR/admin 2>&1 |
     tr '{' "\n" |
...
function testHfAttrs() {
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity add $user $URI --json "$userDef" -H $TESTDIR/$admin 2>&1
   enroll $user $passwd
   expectedAttrs="
...
function testLateralAffiliation() {
   ...
   org=org1
   eval "userDef=\"$adminTemplate\""
   $ZIGLEDGER_CA_CLIENTEXEC identity add $user $URI -d \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity modify $user $URI -d \
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity modify $user $URI -d -H $TESTDIR/$admin \
      --attrs "hf.IntermediateCA=false" 2>&1 ||
         ErrorMsg "Failed to modify user '$user'"
   # restrict hf.Registrar.Attributes for admin
   $ZIGLEDGER_CA_CLIENTEXEC identity modify $admin $URI -d -H $TESTDIR/$admin \
   ...
   eval "userDef=\"$adminTemplate\""
   $ZIGLEDGER_CA_CLIENTEXEC identity add $user $URI -d --json "$userDef" -H $TESTDIR/$admin 2>&1 |
...
function testConflictingHfAttrs() {
   ...
   for flag in  '--type peer' '--affiliation .' '--attrs a=1' '--maxenrollments 1' '--secret p' '--type app'; do
      $ZIGLEDGER_CA_CLIENTEXEC identity add $user $URI -d $flag --json "$userdef" \
         -H $TESTDIR/$admin 2>&1 | grep -o "Can't use 'json' flag" ||
         ErrorMsg "Failed invalid flag combination"
   done

   $ZIGLEDGER_CA_CLIENTEXEC identity add ${user}1 $URI -d --json "$userdef" -H $TESTDIR/$admin 2>&1 |
   ...
   '
   $ZIGLEDGER_CA_CLIENTEXEC identity add ${user}2 $URI -d --json "$userdef" -H $TESTDIR/admin2 2>&1 |
...
function removeAllUsers() {
   ...
   for u in $( $ZIGLEDGER_CA_CLIENTEXEC identity list $URI -H $TESTDIR/admin/ |
                awk '{for (i=1;i<=NR;i++) {gsub(/,/,"");if ($i=="Name:" && $(i+1)!="admin") print $(i+1) }}'); do
      $ZIGLEDGER_CA_CLIENTEXEC identity remove $u $URI -d -H $TESTDIR/admin/ || ErrorMsg "Failed to delete user $u"
   done
   $ZIGLEDGER_CA_CLIENTEXEC identity list $u $URI -H $TESTDIR/admin/
   numUsers=$($ZIGLEDGER_CA_CLIENTEXEC identity list $u $URI -H $TESTDIR/admin/ | wc -l)
   test "$numUsers" -ne 1 && ErrorMsg "Wrong number of users"

   # ensure all user certs revoked
   $SCRIPTDIR/zigledger-ca_setup.sh -L -d mysql -D 2>/dev/null|
   ...
   $ZIGLEDGER_CA_CLIENTEXEC identity remove id $URI -H $TESTDIR/admin/ 2>&1 |
      grepPrint 'Failed to get User' ||
         ErrorMsg "Should have failed, or wrong error code"
   # attempt delete self w/o force
   $ZIGLEDGER_CA_CLIENTEXEC identity remove admin $URI -H $TESTDIR/admin/ 2>&1 |
       grepPrint "Need to use 'force'" ||
          ErrorMsg "Should have failed, or wrong error code"
   # delete self
   $ZIGLEDGER_CA_CLIENTEXEC identity remove admin $URI --force -H $TESTDIR/admin/ ||
       ErrorMsg "Failed to delete self"
   numUsers=$(./scripts/fvt/zigledger-ca_setup.sh -L -d mysql 2>/dev/null|
...
mkdir -p $TESTDIR
$SCRIPTDIR/zigledger-ca_setup.sh -d mysql -I -X -n1 -D -x $TESTDIR
cp $TESTDIR/runZigledgerCaFvt.yaml /tmp
$SCRIPTDIR/zigledger-ca_setup.sh -D -R -x $TESTDIR
mkdir -p $TESTDIR
cp /tmp/runZigledgerCaFvt.yaml  $TESTDIR/runZigledgerCaFvt.yaml
sed -i '/name: admin$/,/hf.Registrar.DelegateRoles:/s/hf.Registrar.Roles:.*/hf.Registrar.Roles: "client,user,peer,validator,auditor,ca,app,role1,role2,role3,role4,role5,role6,role7,role8,apple,orange,ca\"/;
        s/hf.Registrar.DelegateRoles:.*/hf.Registrar.DelegateRoles: "client,user,peer,validator,auditor,ca,app,role1,role2,role3,role4,role5,role6,role7,role8,apple,orange,ca\"/'  $TESTDIR/runZigledgerCaFvt.yaml
$SCRIPTDIR/zigledger-ca_setup.sh -d mysql -S -X -n1 -D -x $TESTDIR -- \
