...
export ZIGLEDGER_CA_SERVER_HOME="/tmp/$TESTCASE"
export CA_CFG_PATH="/tmp/$TESTCASE"

MYSQLSERVERCONFIG="$ZIGLEDGER_CA_SERVER_HOME/mysqlserverconfig.yaml"
MYSQLSERVERCONFIG2="$ZIGLEDGER_CA_SERVER_HOME/mysqlserverconfig2.yaml"
PGSQLSERVERCONFIG="$ZIGLEDGER_CA_SERVER_HOME/pgsqlserverconfig.yaml"
PGSQLSERVERCONFIG2="$ZIGLEDGER_CA_SERVER_HOME/pgsqlserverconfig2.yaml"
MSP="$ZIGLEDGER_CA_SERVER_HOME/msp"
SERVERCERT="$ZIGLEDGER_CA_SERVER_HOME/zigledger-ca-cert.pem"
DBNAME="zigledger_ca"
...
function genConfig {
   ...
   mkdir -p $ZIGLEDGER_CA_SERVER_HOME
...
db:
  type: mysql
  datasource: root:mysql@tcp(localhost:$MYSQL_PORT)/zigledger_ca$mysqlTls
  tls:
...
EOF
   ...
   sed -i "s/type: mysql/type: postgres/
          s/datasource:.*/datasource: host=localhost port=$POSTGRES_PORT user=postgres password=postgres dbname=zigledger_ca $postgresTls/" \
   $PGSQLSERVERCONFIG $PGSQLSERVERCONFIG2
}

$SCRIPTDIR/zigledger-ca_setup.sh -R -x $ZIGLEDGER_CA_SERVER_HOME
genConfig
...
# Zigledger-ca should bootstap a newly added identity to the config to the user table
echo "############## Test 1 ##############"
echo "Test1: Database and tables exist, plus an already bootstrapped user is present in the users table"
echo "Test1: Zigledger-ca should bootstap a newly added identity to the config to the user table"
...
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test1a.txt"
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $MYSQLSERVERCONFIG 2>&1 | tee $SERVERLOG &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
pid=$(pidof zigledger-ca-server)
killserver $pid && rm $SERVERLOG || ErrorExit "Failed to stop CA"
# Starting server second time with a second bootstrap user
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $MYSQLSERVERCONFIG2 2>&1 | tee $SERVERLOG &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
pid=$(pidof zigledger-ca-server)
...
# Zigledger-ca should create the tables and bootstrap
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test2a.txt"
echo "############## Test 2 ##############"
echo "Test2: Database exist but tables do not exist"
echo "Test2: Zigledger-ca should create the tables and bootstrap"
echo "Dropping and creating an empty '$DBNAME' database"
mysql --host=localhost --user=root --password=mysql -e "drop database zigledger_ca;" -e "create database zigledger_ca;" &> /dev/null
...
# Zigledger-ca should create the database and tables, and bootstrap
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test3a.txt"
echo "############## Test 3 ##############"
echo "Test3: Database does not exist"
echo "Test3: Zigledger-ca should create the database and tables, and bootstrap"
echo "Dropping '$DBNAME' database"
mysql --host=localhost --user=root --password=mysql -e "drop database zigledger_ca;" &> /dev/null
...
# Zigledger-ca should create the tables and bootstrap
echo "############## Test 1 ##############"
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test1b.txt"
echo "Test1: Database and tables exist, plus an already bootstrapped user is present in the users table"
echo "Test1: Zigledger-ca should bootstap a newly added identity to the config to the user table"
psql -c "drop database $DBNAME"
psql -c "create database $DBNAME"
psql -d zigledger_ca -c "CREATE TABLE users (id VARCHAR(64), token bytea, type VARCHAR(64), affiliation VARCHAR(64), attributes VARCHAR(256), state INTEGER,  max_enrollments INTEGER)"

# Starting server first time with one bootstrap user
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $PGSQLSERVERCONFIG 2>&1 | tee $SERVERLOG &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
pid=$(pidof zigledger-ca-server)
killserver $pid && rm $SERVERLOG || ErrorExit "Failed to stop CA"

# Starting server second time with a second bootstrap user
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $PGSQLSERVERCONFIG2 2>&1 | tee $SERVERLOG &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
pid=$(pidof zigledger-ca-server)
killserver $pid || ErrorExit "Failed to stop CA"
...
# Zigledger-ca should create the tables and bootstrap
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test2b.txt"
echo "############## Test 2 ##############"
echo "Test2: Database exist but tables do not exist"
echo "Test2: Zigledger-ca should create the tables and bootstrap"
psql -c "drop database $DBNAME"
...
# Test scenario where database does not exist
# Zigledger-ca should create the database and tables, and bootstrap
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test3b.txt"
echo "############## Test 3 ##############"
echo "Test3: Database does not exist"
echo "Test3: Zigledger-ca should create the database and tables, and bootstrap"
psql -c "drop database $DBNAME"
...
# Start zigledger-ca server connecting to postgres, this will fail
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test1c.txt"
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $PGSQLSERVERCONFIG2 | tee $SERVERLOG 2>&1 &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
...
# Start zigledger-ca server connecting to MySQL, this will fail
SERVERLOG="$ZIGLEDGER_CA_SERVER_HOME/serverlog.test2c.txt"
$SCRIPTDIR/zigledger-ca_setup.sh -S -X -g $MYSQLSERVERCONFIG2 | tee $SERVERLOG 2>&1 &
pollLogForMsg "Listening on https*://0.0.0.0:$CA_DEFAULT_PORT" $SERVERLOG || ErrorExit "Failed to log CA startup message"
