...
for driver in mysql postgres sqlite3; do
   ...
   $SCRIPTDIR/zigledger-ca_setup.sh -R -d $driver

   # Setup CA server
   $SCRIPTDIR/zigledger-ca_setup.sh -D -I -d $driver
   genAffYaml >> $CA_CFG_PATH/runZigledgerCaFvt.yaml
   $SCRIPTDIR/zigledger-ca_setup.sh -D -S -X -d $driver -x $CA_CFG_PATH
   if test "$?" -ne 0; then
      kill $HTTP_PID
      wait $HTTP_PID
   ...
   export ZIGLEDGER_CA_CLIENT_HOME="/tmp/revoke_test/${USERS[2]}"
   $ZIGLEDGER_CA_CLIENTEXEC revoke -u $URI -e${USERS[1]} $TLSOPT
   ...
   export ZIGLEDGER_CA_CLIENT_HOME="/tmp/revoke_test/${USERS[0]}"
   ...
   $ZIGLEDGER_CA_CLIENTEXEC revoke -s $SN_UC -a $AKI_UC -u $URI $TLSOPT
   ...
   #$ZIGLEDGER_CA_CLIENTEXEC revoke -s $SN_LC -a $AKI_LC -u $URI -e${USERS[3]}
   echo "=========================> REVOKING by -s -a (LOWERCASE)"
   $ZIGLEDGER_CA_CLIENTEXEC revoke -s $SN_LC -a $AKI_LC -u $URI $TLSOPT

   echo "=========================> REVOKING by --eid"
   export ZIGLEDGER_CA_CLIENT_HOME="/tmp/revoke_test/${USERS[0]}"
   #### Revoke across affiliations not allowed
   $ZIGLEDGER_CA_CLIENTEXEC revoke -u $URI -e${USERS[5]} $TLSOPT

   #### Revoke my own cert
   echo "=========================> REVOKING self"
   $ZIGLEDGER_CA_CLIENTEXEC revoke -e${USERS[0]} -u $URI $TLSOPT
   ...
   export ZIGLEDGER_CA_CLIENT_HOME="/tmp/revoke_test/${USERS[1]}"
