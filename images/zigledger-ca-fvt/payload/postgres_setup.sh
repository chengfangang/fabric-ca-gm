...
cp $ZIGLEDGER_CA_DATA/$TLS_BUNDLE $PGDATA || let RC+=1
cp $ZIGLEDGER_CA_DATA/$TLS_SERVER_CERT $PGDATA || let RC+=1
cp $ZIGLEDGER_CA_DATA/$TLS_SERVER_KEY  $PGDATA || let RC+=1
# postgres insists on restricted access to keys
chown $PGUSER.$PGUSER $PGDATA/*pem || let RC+=1
chmod 600 $PGDATA/ZigledgerTlsServer*.pem || let RC+=1
