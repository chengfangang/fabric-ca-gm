...
# Mysql certificates
cp $ZIGLEDGER_CA_DATA/$TLS_BUNDLE $MYSQLDATA/
cp $ZIGLEDGER_CA_DATA/$TLS_SERVER_CERT $MYSQLDATA/
openssl rsa -in $ZIGLEDGER_CA_DATA/$TLS_SERVER_KEY -out $MYSQLDATA/$TLS_SERVER_KEY || let RC+=1
