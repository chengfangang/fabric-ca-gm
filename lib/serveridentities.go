...
func getIDs(ctx *serverRequestContext, caller spi.User, caname string) error {
	...
	numberOfIdentities := os.Getenv("ZIGLEDGER_CA_SERVER_MAX_IDS_PER_CHUNK")
	...
	if numberOfIdentities == "" {
		...
		if err != nil {
			return newHTTPErr(500, ErrGettingUser, "Incorrect format specified for environment variable 'ZIGLEDGER_CA_SERVER_MAX_IDS_PER_CHUNK', an integer value is required: %s", err)
		}
	}
