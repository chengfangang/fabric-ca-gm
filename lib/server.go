...
import (
	...
	stls "github.com/chengfangang/fabric-ca-gm/lib/tls"
...
const (
	...
	zigledgerCAServerProfilePort = "ZIGLEDGER_CA_SERVER_PROFILE_PORT"
	allRoles                  = "peer,orderer,client,user"
	apiPathPrefix             = "/api/v1/"
)

// Server is the zigledger-ca server
type Server struct {
...
// Init initializes a zigledger-ca server
...
func (s *Server) createDefaultCAConfigs(cacount int) error {
	...
	for i := 1; i <= cacount; i++ {
		...
		cfgFileName := filepath.Join(cahome, "zigledger-ca-config.yaml")

		caName := fmt.Sprintf("ca%d", i)
		cfg := strings.Replace(defaultCACfgTemplate, "<<<CANAME>>>", caName, 1)

		cn := fmt.Sprintf("zigledger-ca-server-ca%d", i)
...
func (s *Server) serve() error {
	...
	if listener == nil {
		...
		// in https://jira.zhigui.org/browse/FAB-3100.
		return nil
	}
...
// checkAndEnableProfiling checks for ZIGLEDGER_CA_SERVER_PROFILE_PORT env variable
...
func (s *Server) checkAndEnableProfiling() error {
	...
	pport := os.Getenv(zigledgerCAServerProfilePort)
	if pport != "" {
...
func (s *Server) autoGenerateTLSCertificateKey() error {
	...
	// Can't use the same CN as the signing certificate CN (default: zigledger-ca-server) otherwise no AKI is generated
