...
func TestGetAllIDs(t *testing.T) {
	...
	os.Setenv("ZIGLEDGER_CA_SERVER_MAX_IDS_PER_CHUNK", "2")
	...
	os.Unsetenv("ZIGLEDGER_CA_SERVER_MAX_IDS_PER_CHUNK")
...
func TestGetID(t *testing.T) {
	...
	// 'admin2' with affiliation of 'org2' is not authorized to get 'testuser' with affiliation of 'zhigui'
	_, err = admin2.GetIdentity("testuser", "")
	assert.Error(t, err, "'admin2' with affiliation of 'org2' is not authorized to get 'testuser' with affiliation of 'zhigui'")
...
func TestDynamicModifyIdentity(t *testing.T) {
	...
	srv := TestGetRootServer(t)
	srv.RegisterBootstrapUser("admin2", "admin2pw", "zhigui")
	err = srv.Start()
	util.FatalError(t, err, "Failed to start server")
	defer srv.Stop()
...
func modifyTypeAndAffiliation(t *testing.T, registry spi.UserRegistry, admin2, admin3 *Identity) {
	...
	// Caller is part of the 'zhigui' affiliation
	modReq.ID = "testuser2"
	...
	modReq.Affiliation = "zhigui.fake"
	_, err = admin2.ModifyIdentity(modReq)
...
func TestDynamicWithMultCA(t *testing.T) {
	...
	srv := TestGetRootServer(t)
	srv.Config.CAfiles = []string{"../../testdata/ca/rootca/ca1/zigledger-ca-server-config.yaml", "../../testdata/ca/rootca/ca2/zigledger-ca-server-config.yaml"}
	err = srv.Start()
	util.FatalError(t, err, "Failed to start server")
	defer srv.Stop()
