...
// ClientConfig is the zigledger-ca client's config
type ClientConfig struct {
	URL        string `def:"http://localhost:7054" opt:"u" help:"URL of zigledger-ca-server"`
...
func (c *ClientConfig) Enroll(rawurl, home string) (*EnrollmentResponse, error) {
	...
	if c.Enrollment.Name == "" {
		...
		return nil, errors.Errorf(
			"The URL of the zigledger CA server is missing the enrollment ID and secret;"+
