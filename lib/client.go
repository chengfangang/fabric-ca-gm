...
// Client is the zigledger-ca client object
type Client struct {
	...
	csp bccsp.BCCSP
	// HTTP client associated with this Zigledger CA client
...
type GetServerInfoResponse struct {
	...
	CAName string
	// CAChain is the PEM-encoded bytes of the zigledger-ca-server's CA chain.
...
func (c *Client) Enroll(req *api.EnrollmentRequest) (*EnrollmentResponse, error) {
	...
	// Send the CSR to the zigledger-ca server with basic auth header
...
// SendReq sends a request to the zigledger-ca-server and fills in the result
...
func (c *Client) CheckEnrollment() error {
	...
	return errors.New("Enrollment information does not exist. Please execute enroll command first. Example: zigledger-ca-client enroll -u http://user:userpw@serverAddr:serverPort")
}
