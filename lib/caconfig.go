...
# A single zigledger-ca-server can service multiple CAs.  The server's configuration
...
# CA name (see "ca.name" below).  Each CA name in the same zigledger-ca-server
...
// The tags are recognized by the RegisterFlags function in zigledger-ca/lib/util.go
// and are as follows:
...
// CAInfo is the CA information on a zigledger-ca-server
type CAInfo struct {
...
type CAConfigDB struct {
	...
	Datasource string `def:"zigledger-ca-server.db" help:"Data source which is database specific"`
...
type ParentServer struct {
	URL    string `opt:"u" help:"URL of the parent zigledger-ca-server (e.g. http://<username>:<password>@<address>:<port)" mask:"url"`
	CAName string `help:"Name of the CA to connect to on zigledger-ca-server"`
}
