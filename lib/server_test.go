...
import (
	...
	libtls "github.com/chengfangang/fabric-ca-gm/lib/tls"
...
const (
	...
	pportEnvVar      = "ZIGLEDGER_CA_SERVER_PROFILE_PORT"
)
...
func TestSRVRootServer(t *testing.T) {
	...
	rr, err = admin.Register(&api.RegistrationRequest{
		Name:        "user1",
		Type:        "user",
		Affiliation: "zhigui.zigledger.security",
	...
	if err != nil {
		...
		assert.Equal(t, "user.zhigui.zigledger.security", ouPath, "Invalid OU path in certificate")
	}
	...
	user2Registration := &api.RegistrationRequest{
		Name:        "user2",
		Type:        "user",
		Affiliation: "zhigui.zigledger-ca",
	}
...
// See https://jira.zhigui.org/projects/FAB/issues/FAB-5188
...
func TestSRVRunningTLSServer(t *testing.T) {
	...
	defer func() {
		...
		err = os.RemoveAll("../testdata/zigledger-ca-server.db")
		if err != nil {
			t.Errorf("RemoveAll failed: %s", err)
		}
		err = os.RemoveAll("../testdata/msp")
...
func TestSRVDefaultDatabase(t *testing.T) {
	...
	exist := util.FileExists("../testdata/zigledger-ca-server.db")
	if !exist {
		t.Error("Failed to create default sqlite zigledger-ca-server.db")
	}
}
...
func TestSRVMultiCAConfigs(t *testing.T) {
	...
	srv := TestGetServer(rootPort, testdataDir, "", -1, t)
	srv.Config.CAfiles = []string{"ca/ca1/zigledger-ca-server-config.yaml", "ca/ca1/zigledger-ca-server-config.yaml", "ca/ca2/zigledger-ca-server-config.yaml"}
	...
	srv.Config.CAfiles = []string{"ca/rootca/ca1/zigledger-ca-server-config.yaml", "ca/rootca/ca2/zigledger-ca-server-config.yaml", "ca/rootca/ca4/zigledger-ca-server-config.yaml"}
	err = srv.Start()
	...
	srv.Config.CAfiles = []string{"ca/rootca/ca1/zigledger-ca-server-config.yaml", "ca/rootca/ca2/zigledger-ca-server-config.yaml", "ca/rootca/ca3/zigledger-ca-server-config.yaml"}
	err = srv.Start()
	...
	err = CopyFile("../testdata/ca/rootca/ca1/zigledger-ca-server-config.yaml", configfile)
	if err != nil {
		t.Errorf("Failed to copy file: %s", err)
	}
	...
	srv.Config.CAfiles = []string{"ca/rootca/ca1/zigledger-ca-server-config.yaml", "ca/rootca/ca2/zigledger-ca-server-config.yaml"}
	...
	if !util.FileExists("../testdata/ca/rootca/ca1/zigledger-ca-server.db") {
		t.Error("Failed to correctly add ca1")
	}

	if !util.FileExists("../testdata/ca/rootca/ca2/zigledger-ca2-server.db") {
...
func TestSRVMultiCAWithIntermediate(t *testing.T) {
	...
	intermediatesrv.Config.CAfiles = []string{"ca/intermediateca/ca1/zigledger-ca-server-config.yaml", "ca/intermediateca/ca2/zigledger-ca-server-config.yaml"}
...
func TestSRVDefaultMultiCA(t *testing.T) {
	...
	srv.Config.CAfiles = []string{"zigledger-ca1-config.yaml"}
...
func TestAutoTLSCertificateGeneration(t *testing.T) {
	...
	srv.Config.TLS.Enabled = true
	srv.Config.TLS.CertFile = "tls-cert.pem"
	srv.Config.CAcfg.CSR.CN = "zigledger-ca-server"
	...
	assert.NoError(t, err, "Error occured during enrollment on TLS enabled zigledger-ca server")
...
func TestRegistrationAffiliation(t *testing.T) {
	...
	server.RegisterBootstrapUser("admin2", "admin2pw", "zhigui")
	err := server.Start()
	...
	assert.NoError(t, err, "Failed to enroll bootstrap user")
	admin2 := eresp.Identity // admin2 has an affiliation of 'zhigui'
	...
	userAff = GetUserAffiliation(user)
	if userAff != "zhigui" {
		t.Errorf("Incorrect affiliation set for user being registered when no affiliation was specified, expected 'zhigui' got %s", userAff)
	}
...
func cleanMultiCADir(t *testing.T) {
	...
	removeFiles := []string{"ca-cert.pem", "ca-key.pem", "zigledger-ca-server.db", "zigledger-ca2-server.db", "ca-chain.pem"}
...
func getServer(port int, home, parentURL string, maxEnroll int, t *testing.T) *Server {
	...
	affiliations := map[string]interface{}{
		"zhigui": map[string]interface{}{
			"zigledger":    []string{"ledger", "orderer", "security"},
			"zigledger-ca": nil,
			"sdk":       nil,
		},
		"org2": nil,
...
func testRegistration(admin *Identity, t *testing.T) {
	...
	topAffiliation := "zhigui"
	midAffiliation := "zhigui.zigledger"
	botAffiliation := "zhigui.zigledger.security"
...
func testBadCryptoData(t *testing.T, s *Server, testData []string) {
	config := []string{"ca/rootca/ca1/zigledger-ca-server-config.yaml"}
