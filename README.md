# Zigledger CA Developer's Guide

This is the Developer's Guide for Zigledger CA, which is a Certificate Authority for Zhigui Zigledger.

See [User's Guide for Zigledger CA](https://zhigui-zigledger-ca.readthedocs.io) for information on how to use Zigledger CA.
...
You are welcome to contribute to Zigledger CA!

The following are guidelines to follow when contributing:

1. See the general information about [contributing to zigledger](http://zhigui-zigledger.readthedocs.io/en/latest/CONTRIBUTING.html).

2. To set up your development environment for doing common development tasks, see [bash_profile](https://github.com/zhigui/zigledger-ca/blob/master/scripts/bash_profile).  This contains variables and functions which can be copied directly into your `.bash_profile` file.  Even if you do not use bash, you should still find the functions instructive.  For example:
   a. **clone** - pulls the latest zigledger-ca code from gerrit and places it based on your GOPATH setting
   b. **cdr** - cd to the zigledger-ca repository root, which is equivalent to "cd $GOPATH/src/github.com/zhigui/zigledger-ca"
...
1. **cmd/zigledger-ca-server** contains the main for the zigledger-ca-server command.
2. **cmd/zigledger-ca-client** contains the main for the zigledger-ca-client command.
...
### Whole server
To enable profiling on the server, set the ZIGLEDGER_CA_SERVER_PROFILE_PORT environment
...
You can start the server in the FVT image by running following docker command from the zigledger-ca root directory:

`docker run -p 8888:8888 -p 8054:8054 -v $PWD:/opt/gopath/src/github.com/zhigui/zigledger-ca -e ZIGLEDGER_CA_SERVER_PROFILE_PORT=8054 --name loadTest -td zhigui/zigledger-ca-fvt test/zigledger-ca-load-tester/launchServer.sh 1`

Then start the load by running `/test/zigledger-ca-load-tester/runLoad.sh -B`
...
`go tool pprof bin/zigledger-ca-server load-cpu.prof`
...
* Remove cfssl from vendor folder
   * cd $GOPATH/src/github.com/zhigui/zigledger-ca/vendor
...
* Remove sqlx package from cfssl vendor folder. This is because certsql.NewAccessor (called by zigledger-ca) requires sqlx.db object to be passed from the same package. If we were to have sqlx package both in zigledger-ca and cfssl vendor folder, go compiler will throw an error
   * rm -rf github.com/cloudflare/cfssl/vendor/github.com/jmoiron/sqlx

* Remove the packages that are added to the zigledger-ca vendor folder that are not needed by zigledger-ca


## License <a name="license"></a>

Zhigui Project source code files are made available under the Apache License, Version 2.0 (Apache-2.0), located in the [LICENSE](LICENSE) file. Zhigui Project documentation files are made available under the Creative Commons Attribution 4.0 International License (CC-BY-4.0), available at http://creativecommons.org/licenses/by/4.0/.
