...
const (
	...
	tdDir                = "../../testdata"
	db                   = "zigledger-ca-server.db"
	serverPort           = 7090
	rootCertEnvVar       = "ZIGLEDGER_CA_CLIENT_TLS_CERTFILES"
	clientKeyEnvVar      = "ZIGLEDGER_CA_CLIENT_TLS_CLIENT_KEYFILE"
	clientCertEnvVar     = "ZIGLEDGER_CA_CLIENT_TLS_CLIENT_CERTFILE"
	moptionDir           = "moption-test"
	clientCMD            = "zigledger-ca-client"
...
var (
	defYaml       string
	zigledgerCADB    = path.Join(tdDir, db)
	srv           *lib.Server
...
func TestCreateDefaultConfigFile(t *testing.T) {
	defYaml = util.GetDefaultConfigFile("zigledger-ca-client")
	os.Remove(defYaml)
...
func TestClientCommandsNoTLS(t *testing.T) {
	...
	err = srv.RegisterBootstrapUser("admin2", "adminpw2", "zhigui")
	if err != nil {
		t.Errorf("Failed to register bootstrap user: %s", err)
	}
	...
	aff["zhigui"] = []string{"org1", "org2", "org3"}
...
func TestEnroll(t *testing.T) {
	...
	// Enroll without -u parameter but with ZIGLEDGER_CA_CLIENT_URL env variable
	// Default client configuration file will be generated. Value of the
	// ZIGLEDGER_CA_CLIENT_URL env variable is used as server URL
	os.Setenv("ZIGLEDGER_CA_CLIENT_URL", enrollURL1)
	defer os.Unsetenv("ZIGLEDGER_CA_CLIENT_URL")
	err = RunMain([]string{cmdName, "enroll", "-d", "-H", adminHome})
	if err != nil {
		t.Errorf("client enroll with ZIGLEDGER_CA_CLIENT_URL env variable failed: %s", err)
	}
...
func testConfigFileTypes(t *testing.T) {
	...
	// these file types are suitable to represent zigledger-ca
...
// TestGetCACert tests zigledger-ca-client getcacert
...
// TestEnroll tests zigledger-ca-client enroll
func testEnroll(t *testing.T) {
...
// TestGencsr tests zigledger-ca-client gencsr
...
`
	writeConfig := func(cn, algo string, size int, dir string) error {
		...
		fileName := filepath.Join(dir, "zigledger-ca-client-config.yaml")
...
// TestReenroll tests zigledger-ca-client reenroll
...
// testRegisterConfigFile tests zigledger-ca-client register using the config file
func testRegisterConfigFile(t *testing.T) {
	...
	err := RunMain([]string{cmdName, "enroll", "-d", "-c",
		"../../testdata/zigledger-ca-client-config.yaml", "-u", enrollURL1})
	if err != nil {
		t.Errorf("client enroll -u failed: %s", err)
	}

	err = RunMain([]string{cmdName, "register", "-d", "-c",
		"../../testdata/zigledger-ca-client-config.yaml"})
	if err != nil {
		t.Errorf("client register failed using config file: %s", err)
	}
}

// testRegisterEnvVar tests zigledger-ca-client register using environment variables
func testRegisterEnvVar(t *testing.T) {
	t.Log("Testing Register command using env variables")

	os.Setenv("ZIGLEDGER_CA_CLIENT_HOME", "../../testdata/")
	os.Setenv("ZIGLEDGER_CA_CLIENT_ID_NAME", "testRegister2")
	os.Setenv("ZIGLEDGER_CA_CLIENT_ID_AFFILIATION", "zhigui.org2")
	os.Setenv("ZIGLEDGER_CA_CLIENT_ID_TYPE", "client")
	defer func() {
		os.Unsetenv("ZIGLEDGER_CA_CLIENT_HOME")
		os.Unsetenv("ZIGLEDGER_CA_CLIENT_ID_NAME")
		os.Unsetenv("ZIGLEDGER_CA_CLIENT_ID_AFFILIATION")
		os.Unsetenv("ZIGLEDGER_CA_CLIENT_ID_TYPE")
	}()
...
// testRegisterCommandLine tests zigledger-ca-client register using command line input
func testRegisterCommandLine(t *testing.T, srv *lib.Server) {
	t.Log("Testing Register using command line options")
	os.Setenv("ZIGLEDGER_CA_CLIENT_HOME", "../../testdata/")
	defer os.Unsetenv("ZIGLEDGER_CA_CLIENT_HOME")
	...
	err := RunMain([]string{cmdName, "register", "-d", "--id.name", "testRegister3",
		"--id.affiliation", "zhigui.org1", "--id.type", "client", "--id.attrs",
		attributes})
	if err != nil {
		t.Errorf("client register failed: %s", err)
	}
	...
	err = RunMain([]string{cmdName, "register", "-d", "--id.name", "testRegister4",
		"--id.secret", "testRegister4", "--id.affiliation", "zhigui.org2", "--id.type", "user"})
	if err != nil {
		t.Errorf("client register failed: %s", err)
	}
	...
	err = RunMain([]string{cmdName, "register", "-d", "--id.name", userName,
		"--id.secret", "testRegister5", "--id.affiliation", "zhigui.org1"})
...
// TestRevoke tests zigledger-ca-client revoke
func testRevoke(t *testing.T) {
	t.Log("Testing Revoke command")
	clientHome := "../../testdata/"
	os.Setenv("ZIGLEDGER_CA_CLIENT_HOME", clientHome)
	defer os.Unsetenv("ZIGLEDGER_CA_CLIENT_HOME")
	...
	// revoker's affiliation: zhigui, revoking affiliation: ""
	err = RunMain([]string{cmdName, "revoke", "-u", serverURL,
	...
	// zigledger ca server returns an error if the serial and aki do not belong
	...
	// testRegister2's affiliation: zhigui.org2, zhigui.org2
	...
	// Revoked user's affiliation: zhigui.org3
	err = RunMain([]string{cmdName, "revoke", "-u", serverURL, "--revoke.name",
...
func testAffiliation(t *testing.T) {
	...
	// admin2 has affiliation of 'zhigui'
	...
	// Registering with affiliation of "zhigui", valid scenario
	err = RunMain([]string{cmdName, "register", "-d", "--id.name", "testRegister6", "--id.type", "client", "--id.affiliation", "zhigui"})
	if err != nil {
		t.Errorf("client register failed: %s", err)
	}
	...
	if userAff != "zhigui" {
		t.Errorf("Incorrectly set affiliation for user being registered when no affiliation was specified, expected 'zhigui' got %s", userAff)
	}

	os.RemoveAll(filepath.Dir(defYaml))
}

// testProfiling tests enablement of zigledger CA client heap/cpu profiling
func testProfiling(t *testing.T) {
	...
	for _, testCase := range testCases {
		os.Setenv(zigledgerCAClientProfileMode, testCase.pEnvVal)
	...
	os.Unsetenv(zigledgerCAClientProfileMode)
}
...
func TestClientCommandsUsingConfigFile(t *testing.T) {
	...
	err = RunMain([]string{cmdName, "enroll", "-c",
		"../../testdata/zigledger-ca-client-config.yaml", "-u",
		tlsEnrollURL, "-d"})
	if err != nil {
...
func TestMultiCA(t *testing.T) {
	...
	srv.Config.CAfiles = []string{"ca/rootca/ca1/zigledger-ca-server-config.yaml",
		"ca/rootca/ca2/zigledger-ca-server-config.yaml"}
...
func TestHomeDirectory(t *testing.T) {
	...
	if !util.FileExists(configFilePath) {
		t.Errorf("Failed to correctly created the default config (zigledger-ca-client-config) in the default home directory")
	}
	...
	if !util.FileExists(filepath.Join("../../testdata/testhome/testclientcmd", defaultClientConfigFile)) {
		t.Errorf("Failed to correctly created the default config (zigledger-ca-client-config.yaml) in the '../../testdata/testhome/testclientcmd' directory")
	}
...
func cleanMultiCADir() {
	...
	removeFiles := []string{"msp", "ca-cert.pem",
		"zigledger-ca-server.db", "zigledger-ca2-server.db", "ca-chain.pem"}
...
func testGetCACertConfigFile(t *testing.T) error {
	...
	configFile := "../../testdata/zigledger-ca-client-config.yaml"
...
func TestVersion(t *testing.T) {
	...
	if err != nil {
		t.Error("Failed to get zigledger-ca-client version: ", err)
	}
}
