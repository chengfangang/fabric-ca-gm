...
const (
	longName     = "Zhigui Zigledger Certificate Authority Client"
	shortName    = "zigledger-ca client"
	cmdName      = "zigledger-ca-client"
	envVarPrefix = "ZIGLEDGER_CA_CLIENT"
	homeEnvVar   = "ZIGLEDGER_CA_CLIENT_HOME"
)
...
#         To set the zigledger-ca server url
#      b) ZIGLEDGER_CA_CLIENT_TLS_CLIENT_CERTFILE=certfile.pem
#         To set the client certificate for TLS
...
#############################################################################
#  Registration section used to register a new identity with zigledger-ca server
#
...
#############################################################################
#  Enrollment section used to enroll an identity with zigledger-ca server
#
...
#############################################################################
# Name of the CA to connect to within the zigledger-ca server
#############################################################################
caname:
...
func (c *ClientCmd) createDefaultConfigFile() error {
	...
	var cfg string
	zigledgerCAServerURL := c.myViper.GetString("url")
	if zigledgerCAServerURL == "" {
		zigledgerCAServerURL = util.GetServerURL()
	} else {
		URL, err := url.Parse(zigledgerCAServerURL)
		if err != nil {
			return errors.Wrapf(err, "Failed to parse URL '%s'", zigledgerCAServerURL)
		}
		zigledgerCAServerURL = fmt.Sprintf("%s://%s", URL.Scheme, URL.Host)
	}

	myhost := c.myViper.GetString("myhost")

	// Do string subtitution to get the default config
	cfg = strings.Replace(defaultCfgTemplate, "<<<URL>>>", zigledgerCAServerURL, 1)
