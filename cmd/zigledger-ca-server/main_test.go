...
// based on the arguments passed to the zigledger-ca-server and environment variables
func TestDBLocation(t *testing.T) {
	blockingStart = false
	envs := []string{"ZIGLEDGER_CA_SERVER_HOME", "ZIGLEDGER_CA_HOME", "CA_CFG_PATH",
		"ZIGLEDGER_CA_SERVER_DB_DATASOURCE"}
	...
	dsFile := "serverConfig/zigledger-ca-server.db"
	...
	// and ZIGLEDGER_CA_SERVER_DB_DATASOURCE env variable set to zigledger-ca-srv.db (relative path)
	os.Setenv("ZIGLEDGER_CA_SERVER_DB_DATASOURCE", "zigledger-ca-srv.db")
	cfgFile = "serverConfig1/config.yml"
	dsFile = "serverConfig1/zigledger-ca-srv.db"
	...
	// and ZIGLEDGER_CA_SERVER_DB_DATASOURCE env variable set to /tmp/zigledger-ca-srv.db (absolute path)
	cfgFile = "serverConfig2/config.yml"
	dsFile = os.TempDir() + "/zigledger-ca-srv.db"
	os.Setenv("ZIGLEDGER_CA_SERVER_DB_DATASOURCE", dsFile)
	...
	os.RemoveAll(os.TempDir() + "/config")
	os.Remove(dsFile)
	os.Unsetenv("ZIGLEDGER_CA_SERVER_DB_DATASOURCE")
}
...
func TestDefaultMultiCAs(t *testing.T) {
	...
	if !util.FileExists("ca/ca4/zigledger-ca-server_ca4.db") {
...
func TestMultiCA(t *testing.T) {
	...
	err := RunMain([]string{cmdName, "start", "-d", "-p", "7056", "-c", "../../testdata/test.yaml", "-b", "user:pass", "--cafiles", "ca/rootca/ca1/zigledger-ca-server-config.yaml", "--cafiles", "ca/rootca/ca2/zigledger-ca-server-config.yaml"})
	if err != nil {
	...
	err = RunMain([]string{cmdName, "start", "-d", "-p", "7056", "-c", "../../testdata/test.yaml", "-b", "user:pass", "--cacount", "1", "--cafiles", "ca/rootca/ca1/zigledger-ca-server-config.yaml", "--cafiles", "ca/rootca/ca2/zigledger-ca-server-config.yaml"})
	if err == nil {
...
func TestRegistrarAttribute(t *testing.T) {
	...
	err = os.Setenv("ZIGLEDGER_CA_SERVER_HOME", "testregattr/server")
...
func TestVersion(t *testing.T) {
	...
	if err != nil {
		t.Error("Failed to get zigledger-ca-server version: ", err)
	}
}
...
func TestClean(t *testing.T) {
	...
	os.Remove("zigledger-ca-server.db")
	os.RemoveAll("keystore")
	os.RemoveAll("msp")
	os.RemoveAll("../../testdata/msp")
	os.Remove("../../testdata/zigledger-ca-server.db")
...
func cleanUpMultiCAFiles() {
	...
	removeFiles := []string{"msp", "ca-cert.pem", "ca-key.pem", "zigledger-ca-server.db", "zigledger-ca2-server.db"}
