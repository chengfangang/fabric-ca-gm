...
const (
	longName     = "Zhigui Zigledger Certificate Authority Server"
	shortName    = "zigledger-ca server"
	cmdName      = "zigledger-ca-server"
	envVarPrefix = "ZIGLEDGER_CA_SERVER"
	homeEnvVar   = "ZIGLEDGER_CA_SERVER_HOME"
	caNameReqMsg = "ca.name property is required but is missing from the configuration file"
)

const (
	defaultCfgTemplate = `#############################################################################
#   This is a configuration file for the zigledger-ca-server command.
#
...
#      Examples:
#      a) ZIGLEDGER_CA_SERVER_PORT=443
#         To set the listening port
#      b) ZIGLEDGER_CA_SERVER_CA_KEYFILE="../mykey.pem"
#         To set the "keyfile" element in the "ca" section below;
...
#  may not be used if you want to run the zigledger-ca-server in a cluster.
#  To run the zigledger-ca-server in a cluster, you must choose "postgres"
#  or "mysql".
#############################################################################
db:
  type: sqlite3
  datasource: zigledger-ca-server.db
  tls:
      enabled: false
      certfiles:
      client:
...
#############################################################################
#  LDAP section
#  If LDAP is enabled, the zigledger-ca-server calls LDAP to:
...
ldap:
   ...
   tls:
      ...
      client:
         certfile:
         keyfile:
   # Attribute related configuration for mapping from LDAP entries to Zigledger CA attributes
   attribute:
      # 'names' is an array of strings containing the LDAP attribute names which are
      # requested from the LDAP server for an LDAP identity's entry
      names: ['uid','member']
      # The 'converters' section is used to convert an LDAP entry to the value of
      # a zigledger CA attribute.
      # For example, the following converts an LDAP 'uid' attribute
      # whose value begins with 'revoker' to a zigledger CA attribute
...
#############################################################################
# Affiliations section. Zigledger CA server can be bootstrapped with the
...
# Multi CA section
#
# Each Zigledger CA server contains one CA by default.  This section is used
...
#
# Examples:
# zigledger-ca-server start -b admin:adminpw --cacount 2
#
# zigledger-ca-server start -b admin:adminpw --cafiles ca/ca1/zigledger-ca-server-config.yaml
# --cafiles ca/ca2/zigledger-ca-server-config.yaml
#
#############################################################################
...
func (s *ServerCmd) createDefaultConfigFile() error {
	...
	if !ldapEnabled {
		// When LDAP is disabled, the zigledger-ca-server functions as its own
	...
	if purl == "" {
		// This is a root CA
		cfg = strings.Replace(cfg, "<<<COMMONNAME>>>", "zigledger-ca-server", 1)
