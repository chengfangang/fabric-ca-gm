...
func TestGetDefaultConfigFile(t *testing.T) {
	os.Unsetenv("ZIGLEDGER_CA_HOME")
	os.Unsetenv("ZIGLEDGER_CA_CLIENT_HOME")
	os.Unsetenv("ZIGLEDGER_CA_SERVER_HOME")
	os.Unsetenv("CA_CFG_PATH")

	const clientConfig = "zigledger-ca-client-config.yaml"
	const serverConfig = "zigledger-ca-server-config.yaml"

	os.Setenv("HOME", "/tmp")

	expected := filepath.Join("/tmp/.zigledger-ca-client/", clientConfig)
	real := GetDefaultConfigFile("zigledger-ca-client")
	if real != expected {
		t.Errorf("Incorrect default config path retrieved; expected %s but found %s",
			expected, real)
	}

	os.Setenv("ZIGLEDGER_CA_HOME", "/tmp")
	...
	real = GetDefaultConfigFile("zigledger-ca-server")
	if real != expected {
		t.Errorf("Incorrect default config path retrieved; expected %s but found %s",
			expected, real)
	}

	os.Setenv("ZIGLEDGER_CA_CLIENT_HOME", "/tmp/client")
	...
	os.Setenv("ZIGLEDGER_CA_SERVER_HOME", "/tmp/server")
...
func TestGetUser(t *testing.T) {
	os.Unsetenv("ZIGLEDGER_CA_CLIENT_URL")
	viper.BindEnv("url", "ZIGLEDGER_CA_CLIENT_URL")
	os.Setenv("ZIGLEDGER_CA_CLIENT_URL", "http://localhost:7054")
	_, _, err := GetUser(viper.GetViper())
	assert.Error(t, err, "Should have failed no username and password provided")

	os.Setenv("ZIGLEDGER_CA_CLIENT_URL", "http://:pass@localhost:7054")
	_, _, err = GetUser(viper.GetViper())
	assert.Error(t, err, "Should have failed no username provided")

	os.Setenv("ZIGLEDGER_CA_CLIENT_URL", "http://user:@localhost:7054")
	_, _, err = GetUser(viper.GetViper())
	assert.Error(t, err, "Should have failed no password provided")

	os.Setenv("ZIGLEDGER_CA_CLIENT_URL", "http://foo:bar@localhost:7054")
...
func TestValidateAndReturnAbsConf(t *testing.T) {
	...
	filename, _, err = ValidateAndReturnAbsConf("/tmp/test.yaml", "/tmp/homeDir", "zigledger-ca-client")
	...
	filename, homeDir, err = ValidateAndReturnAbsConf("", "../testdata/tmp", "zigledger-ca-client")
	assert.NoError(t, err, "Should not have errored out, this is a valid configuration")
	...
	if filename != filepath.Join(homeDirAbs, "zigledger-ca-client-config.yaml") {
		t.Error("Failed to get correct path for configuration file")
	}

	// Test with no home directory set
	filename, _, err = ValidateAndReturnAbsConf("/tmp/test.yaml", "", "zigledger-ca-client")
	assert.NoError(t, err, "Should not have errored out, this is a valid configuration")
	...
	filename, homeDir, err = ValidateAndReturnAbsConf("../testdata/tmp/test.yaml", "", "zigledger-ca-client")
	assert.NoError(t, err, "Should not have errored out, this is a valid configuration")
