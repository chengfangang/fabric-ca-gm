...
func CreateClientHome() (string, error) {
	...
	home := filepath.Dir(GetDefaultConfigFile("zigledger-ca-client"))
...
func GetDefaultConfigFile(cmdName string) string {
	if cmdName == "zigledger-ca-server" {
	...
	// First check home env variables
	var home string
	envs := []string{"ZIGLEDGER_CA_CLIENT_HOME", "ZIGLEDGER_CA_HOME", "CA_CFG_PATH"}
	for _, env := range envs {
		envVal := os.Getenv(env)
		if envVal != "" {
			home = envVal
	...
	return path.Join(os.Getenv("HOME"), ".zigledger-ca-client", fname)
}
...
func GetUser(v *viper.Viper) (string, string, error) {
	var zigledgerCAServerURL string
	zigledgerCAServerURL = v.GetString("url")
	...
	user := URL.User
	if user == nil {
		return "", "", errors.New("No username and password provided as part of the Zigledger CA server URL")
	}
