...
func TestGetCommandLineOptValue(t *testing.T) {
	testGetCommandLineOptValue(t,
		[]string{"zigledger-ca", "client", "enroll", "-config", "myconfig.json"},
		"-config",
		true,
		"myconfig.json",
		[]string{"zigledger-ca", "client", "enroll"})
	testGetCommandLineOptValue(t,
		[]string{"zigledger-ca", "client", "-config", "myconfig.json", "enroll"},
		"-config",
		true,
		"myconfig.json",
		[]string{"zigledger-ca", "client", "enroll"})
...
func TestSetDefaultServerPort(t *testing.T) {
	testSetDefaultServerPort(t,
		[]string{"zigledger-ca", "client", "enroll"},
		[]string{"zigledger-ca", "client", "enroll", "-port", "7054"})
	testSetDefaultServerPort(t,
		[]string{"zigledger-ca", "client", "enroll", "-port", "1234"},
		[]string{"zigledger-ca", "client", "enroll", "-port", "1234"})
}
...
func testOpt(t *testing.T, opt, val, expectedVal string) {
	saveArgs := os.Args
	if val != "" {
		os.Args = []string{"zigledger-ca", "client", "enroll", opt, val}
	} else {
		os.Args = []string{"zigledger-ca", "client", "enroll"}
	}
	switch opt {
	case "-protocol":
